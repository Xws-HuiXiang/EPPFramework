...
namespace EPPFServer.Common
{
    ...
    public class MessageUtils
    {
        ...
        /// <returns></returns>
        public static byte[] EncodeMsg(int protocolHandleID, int protocolHandleMethodID, MsgBase msg)
        {
            ...
            if (protocolHandleID == (int)MsgHandle.Secret && protocolHandleMethodID == (int)MsgSecretKeyHandleMethod.GetSecretKey)
            {
                msgByteArray = AES.AESEncrypt(array, ServerSocket.PublicKey);
            }
            else
            {
                msgByteArray = AES.AESEncrypt(array, ServerSocket.SecretKey);
            }
        ...
        {
            return EncodeMsg(msg.ProtocolHandleID, msg.ProtocolHandleMethodID, msg);
        }
        ...
        public static void DecodeMsg(MessageUtils messageUtils, out int protocolHandleID, out int protocolHandleMethodID, out byte[] msgByteArray)
        {
            ...
            {
                Debug.L.Error(string.Format("消息解析“处理类ID”时出错：" + e.Message));
            ...
            {
                Debug.L.Error(string.Format("消息解析“处理类中方法的ID”时出错：" + e.Message));
            ...
            try
            {
                ...
                Array.Copy(data, messageUtils.readIndex, newData, 0, contentLength);
                //如果是获取密钥的协议，则使用公钥解密，否则使用密钥解密
                string key = ServerSocket.SecretKey;
                if(protocolHandleID == (int)MsgHandle.Secret && protocolHandleMethodID == (int)MsgSecretKeyHandleMethod.GetSecretKey)
                {
                    key = ServerSocket.PublicKey;
                }
                msgByteArray = AES.AESDecrypt(newData, key);
                
                messageUtils.SetReadIndex(messageUtils.ReadIndex + contentLength);
            }
            catch (Exception e)
            {
                Debug.L.Error(string.Format("消息解析“消息内容”时出错：" + e.Message));
